...
describe('Rebrickable Service', () => {
  ...
  test('should return list of colors', done => {
    ...
    const req = httpTestingController.expectOne('https://rebrickable.com/api/v3/lego/colors/?ordering=-name%2Cid&page=2&page_size=2');
    expect(req.request.method).toEqual('GET');
    req.flush(DbMocks.Colors);
  });

  test('should return a color', done => {
    rebrickableService.color(1, {
        ordering: {
          type: 'DESC',
          fields: ['name', 'id']
        }
  ...
  test('should return a list of minifigs', done => {
    ...
    const req = httpTestingController.expectOne('https://rebrickable.com/api/v3/lego/minifigs/?in_set_num=100&in_theme_id=100&max_parts=3&min_parts=1&ordering=name&page=2&page_size=10&search=query');
    expect(req.request.method).toEqual('GET')
    req.flush(DbMocks.Minifigs)
  });

  test('should return a minifig', done => {
    rebrickableService.minifig('fig-000018')
      .subscribe(minifig => {
